...
def _load_teacher(args):
    ...
    text_kd_active = (
        getattr(args, "lambda_text_kd", 0.0) > 0
        and getattr(args, "text_kd_mode", "none") != "none"
    )
    if args.lambda_kd <= 0 and not args.joint_teacher_adapter and not text_kd_active:
...
class CustomCLIP(nn.Module):
    def __init__(
        ...
        print(
            ...
            f"text_kd={getattr(cfg, 'lambda_text_kd', 0.0)}, "
            f"text_kd_mode={getattr(cfg, 'text_kd_mode', 'none')}"
        )
    ...
    def forward(self, x, classnames):
        ...
        if self.teacher_active:
            ...
            text_kd_active = (
                getattr(self.cfg, "lambda_text_kd", 0.0) > 0
                and getattr(self.cfg, "text_kd_mode", "none") != "none"
            )
            if self.joint_teacher_adapter or text_kd_active:
...
class ZS_SBIR(pl.LightningModule):
    ...
    def _print_model_debug_summary(self):
        ...
        print(
            ...
            f"topk_text_kd={getattr(self.args, 'lambda_text_kd', 0.0)}, "
            f"text_kd_mode={getattr(self.args, 'text_kd_mode', 'none')}, "
